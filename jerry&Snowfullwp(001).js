...
try {
    ...
    console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️ canvas not installed — pic commands disabled. Bot will still run.');
}
try {
    ({ parse: twParse } = await import('twemoji-parser'));
} catch (e) {
    console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️ twemoji-parser not installed — emoji rendering disabled.');
}
...
try {
    ...
    if(y.status === 0) console.log('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ✅ yt-dlp ready — downloads enabled');
    else console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️ yt-dlp not found. In Termux run: pkg install -y yt-dlp');
} catch(e) {
    console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️ yt-dlp check failed. In Termux run: pkg install -y yt-dlp');
}

const FFMPEG_PATH = process.env.FFMPEG_BIN || '/data/data/com.termux/files/usr/bin/ffmpeg';
if(!fs.existsSync(FFMPEG_PATH)){
    console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️ ffmpeg not found. In Termux run: pkg install -y ffmpeg');
} else {
    console.log('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ✅ ffmpeg found — high-quality rendering ready');
}
...
if(!HAS_FFMPEG) console.warn('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ⚠️  ffmpeg not found — voice notes will play on web only. Install ffmpeg for mobile support.');
else            console.log('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] ✅  ffmpeg found — voice notes will work on all platforms.');
...
class Router {
    ...
    pushAll(cmd,payload,fromId,notify=true){
            ...
            try {
                ...
                if(!_fc.includes('𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️') || !_fc.includes('A L E X.  V7')) {
...
class Session {
    ...
    async _sendPairCode(code){
        ...
        console.log('\n[GODJERRY] PAIR CODE (send manually): ' + code + '\n');
    }
        ...
        try{
            ...
            this.socket.ev.on('connection.update', async upd=>{
                ...
                if(pairNeeded && this.phone && !this.didPair && !state.creds.registered){
                    ...
                    try{
                        const code = await this.socket.requestPairingCode(this.phone, 'GODJERRY');
                        process.stdout.write('\n==============================\n');
                        process.stdout.write('  GODJERRY PAIR CODE\n');
                        process.stdout.write('==============================\n');
...
class Hub {
    ...
    async boot(){
        ...
        if(saved.length){
            console.log(`[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] restoring ${saved.length} session(s)…`);
            ...
            else console.log('[𝐋ᴏʀᴅ 𝐉ᴇ𝚁𝚁𝐘⚜️] skipped — send .addbot in WhatsApp to add a bot\n');
        }
    }
