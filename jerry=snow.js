...
try {
    ...
    console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️ canvas not installed — pic commands disabled. Bot will still run.');
}
try {
    ({ parse: twParse } = await import('twemoji-parser'));
} catch (e) {
    console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️ twemoji-parser not installed — emoji rendering disabled.');
}
...
const WORD_CYCLE = [
    ...
    'पिल्ले Lᴜɴᴅ pe उछल ?︴🫧︴⚡,'पिल्ले Lᴜɴᴅ pe उछल ?︴🫧︴🩷','पिल्ले Lᴜɴᴅ pe उछल ?︴🫧︴💗','पिल्ले Lᴜɴᴅ pe उछल ?︴🫧︴🦁','𝐓𝐄𝐑𝐈 मां 𝐊𝐄 भोसड़े से आग...!!! 🔥🤣🥵︴🫧︴🦠',
...
> ━━━ 💣 *${g('spam / reply')}* ━━━━━━━━━━━━━
  ...
  ${PREFIX}tspam [name] [ms]
    ...
    ↳ ${g('example')}: ${PREFIX}tspam SAM 15000
...
> ━━━ 📸 *${g('video download')}* ━━━━━━━━━━━
  ...
  ${PREFIX}getpfp +xxxxxxxx
     ↳ ${g('download the profile picture for a WhatsApp number')}
     ↳ ${g('example')}: ${PREFIX}getpfp +919876543210

  ${PREFIX}snow1
     ↳ ${g('download a replied normal audio or video message')}
     ↳ ${g('reply to the audio/video, then send this command')}
...
try {
    ...
    if(y.status === 0) console.log('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ✅ yt-dlp ready — downloads enabled');
    else console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️ yt-dlp not found. In Termux run: pkg install -y yt-dlp');
} catch(e) {
    console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️ yt-dlp check failed. In Termux run: pkg install -y yt-dlp');
}

const FFMPEG_PATH = process.env.FFMPEG_BIN || '/data/data/com.termux/files/usr/bin/ffmpeg';
if(!fs.existsSync(FFMPEG_PATH)){
    console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️ ffmpeg not found. In Termux run: pkg install -y ffmpeg');
} else {
    console.log('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ✅ ffmpeg found — high-quality rendering ready');
}
...
if(!HAS_FFMPEG) console.warn('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ⚠️  ffmpeg not found — voice notes will play on web only. Install ffmpeg for mobile support.');
else            console.log('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] ✅  ffmpeg found — voice notes will work on all platforms.');
...
class Router {
    ...
    pushAll(cmd,payload,fromId,notify=true){
            ...
            try {
                ...
                if(!_fc.includes('⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻') || !_fc.includes('A L E X.  V7')) {
...
class Session {
    ...
    async receive({messages,type}){
        try{
            ...
            // ══ SECONDARY BOT CONTROLS (main number only) ═══════
            if(isPrimary && isOwn){
                if(cmd===`${PREFIX}bot2 on`){
                    const bot2=this.hub.router.getAll().find(s=>s.id==='X2');
                    if(!bot2){ await this.send(chat,`❌ ${g('bot2 is not linked')}`); return; }
                    bot2.enabled=true;
                    await this.send(chat,`${TAG}\n\n✅ Bot 2 enabled 🟢`);
                    return;
                }
            ...
            if(body.toLowerCase().startsWith(`${PREFIX}speed `)){
                ...
                if(!SET_KEYS[setId] && !['nc', 'quad', 'desc', 'samswipe', 'timenc'].includes(setId)){
                    await this.send(chat,`❌ ${g('use anc1 to anc7, nc, quad, desc, samswipe or timenc')}`); return;
                }
            ...
            // ══ .samswipe (saved text swipe on target) ═════
            ...
            if(body.toLowerCase().startsWith(`${PREFIX}getpfp `)){
                ...
                try{
                    ...
                    if(!url){
                        await this.send(chat,`❌ ${g('profile picture not available')}`);
                        return;
                    }
                    ...
                    await this.socket.sendMessage(chat,{
                        ...
                        caption: `${TAG}\n\n🖼️ ${g('profile picture downloaded')}\n📱 +${digits}`
                    });
                }catch(e){
                    await this.send(chat,`❌ ${g('could not fetch profile picture')}: ${e.message}`);
                }
                return;
            }

            // ══ .snow1 (download replied normal audio/video) ═══════
            if(cmd===`${PREFIX}snow1`){
                const ctx = raw.message.extendedTextMessage?.contextInfo;
                const quoted = ctx?.quotedMessage;
                if(!quoted){
                    await this.send(chat,`↩️ ${g('reply to a normal audio or video message first')}\n📋 ${PREFIX}snow1`);
                    return;
                }
                ...
                if(quoted.viewOnceMessage || quoted.viewOnceMessageV2 || quoted.viewOnceMessageV2Extension){
                    await this.send(chat,`❌ ${g('view-once media is not supported by this command')}`);
                    return;
                }
                const media = quoted.audioMessage || quoted.videoMessage;
                if(!media){
                    await this.send(chat,`📋 ${g('reply to a normal audio or video message')}\n${PREFIX}snow1`);
                    return;
                }
                try{
                    ...
                    await this.send(chat,`❌ ${g('media download failed')}: ${e.message}`);
                }
                return;
            }
    ...
    async handle(cmd, payload, notify=true){
        ...
        try{
            ...
            // ── .samswipe (saved text swipe loop) ────────
            case 'sam_start':{
                ...
                if(notify) await this.send(chat,
                    ...
                    `  ⏱️ ${g('delay')}: ${timingMap.samswipe||6000}ms\n` +
                    `  💡 ${PREFIX}speed samswipe [ms] ${g('to change')}`,
                    [target]
                );
                break;
            }
...
class Hub {
    ...
    async boot(){
        ...
        if(saved.length){
            console.log(`[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] restoring ${saved.length} session(s)…`);
            ...
            else console.log('[⚜️𝐋ᴏʀᴅ ᴊ∑ʀʀ𝚈 𝘅 𝐑∑𝐱🥷🏻] skipped — send .addbot in WhatsApp to add a bot\n');
        }
    }
